...
function SourceCard({ source, onCollect, disabled }: { source: Source; onCollect: (source: Source) => void; disabled: boolean }) {
  return (
    <article className="rounded-3xl border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-slate-950/30">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <button type="button" disabled={disabled} onClick={() => onCollect(source)} className="rounded-2xl bg-cyan-300 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-cyan-200 disabled:cursor-not-allowed disabled:bg-slate-600 disabled:text-slate-300">Собрать RSS</button>
      </div>
...
export default function SourcesPage() {
  ...
  const collectRss = useCollectRss({ onSuccess: (data) => showToast({ title: "Сбор RSS завершён", description: `Создано новых материалов: ${data.created ?? 0}. Пропущено: ${data.skipped ?? 0}.` }), onError: (error) => showToast({ title: "Не удалось собрать RSS", description: getRussianErrorMessage(error) }) });
