...
function validateForm(form: FormState) {
  ...
  if (!/^[a-zа-яё-]{2,16}$/i.test(form.language.trim()))
    errors.language = "Укажите язык от 2 до 16 символов, например ru или en.";
  return errors;
}
...
function SourceCard({
  ...
  const disabledReason = !isActive
    ? `Сбор недоступен: источник в статусе «${statusText(source.status)}», нужен статус «Активен».`
    : disabled && !isCollecting
      ? "Сбор уже запущен для другого источника. Дождитесь завершения."
      : "Действие доступно: будут загружены новые RSS-материалы.";
  return (
    <article className="rounded-3xl border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-slate-950/30">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="space-y-2 lg:max-w-sm">
          <ActionButton
            ...
            loadingText="Собираем RSS…"
            onClick={() => onCollect(source)}
          >
            Собрать RSS
          </ActionButton>
          <InlineNotice tone={isActive && !disabled ? "info" : "warning"}>
            {disabledReason}
          </InlineNotice>
        </div>
      </div>
      <dl className="mt-5 grid gap-3 text-sm md:grid-cols-4">
        ...
        <div>
          ...
          <dd className="mt-1 text-slate-100">{source.language}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Тема</dt>
          <dd className="mt-1 text-slate-100">
            {source.topic || "Не указана"}
          </dd>
        </div>
        <div>
...
export default function SourcesPage() {
  ...
  const createSource = useCreateSource({
    onSuccess: () => {
      ...
      showToast({
        ...
        description: "RSS-источник сохранён и появился в списке.",
      });
    },
    onError: (error) =>
      showToast({
  ...
  const [collectingSourceId, setCollectingSourceId] = useState<number | null>(
    null,
  );
  ...
  const totalText = useMemo(
    () =>
      `Показано источников: ${sources.length} из ${sourcesQuery.data?.total ?? sources.length}.`,
    [sources.length, sourcesQuery.data?.total],
  );
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (Object.keys(nextErrors).length > 0) {
      showToast({
        ...
        description: "Исправьте ошибки в полях и попробуйте снова.",
      });
      return;
    }
    createSource.mutate({
    ...
    <main className="min-h-screen bg-slate-950 p-5 text-slate-100">
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="rounded-3xl border border-white/10 bg-white/[0.06] p-6">
          ...
          <h1 className="mt-3 text-3xl font-bold text-white">Источники RSS</h1>
          <p className="mt-2 text-slate-400">
            Добавляйте ленты, запускайте сбор и контролируйте список источников.
          </p>
        </header>
        <section className="grid gap-5 lg:grid-cols-[24rem_1fr]">
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-400">
              Все поля проверяются в браузере до отправки.
            </p>
            <div className="mt-5 grid gap-4">
              {(
                [
                  ...
                  { key: "language", label: "Язык", placeholder: "ru" },
                  { key: "topic", label: "Тема", placeholder: "Опционально" },
                  {
                    key: "strategy",
                    label: "Стратегия",
                ...
                >
                  ...
                  <input
                    ...
                    className="rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-white outline-none placeholder:text-slate-600 focus:border-cyan-300/60"
                  />
                  {errors[field.key] && (
                    <span className="text-rose-200">{errors[field.key]}</span>
                  )}
                </label>
              ))}
            </div>
            <button
              ...
              className="mt-5 w-full rounded-2xl bg-cyan-300 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-cyan-200 disabled:bg-slate-600 disabled:text-slate-300"
            >
              {createSource.isPending ? "Добавляем…" : "Добавить источник"}
            </button>
          </form>
          <section className="space-y-3">
            <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-5">
              ...
              <p className="mt-1 text-sm text-slate-400">{totalText}</p>
            </div>
            ...
            {sourcesQuery.error && (
              <div className="rounded-3xl border border-rose-300/30 bg-rose-400/10 p-5 text-rose-100">
                ...
                {getRussianErrorMessage(sourcesQuery.error)}
              </div>
            )}
            {collectRss.isPending && (
              <InlineNotice tone="info">
                Сбор RSS выполняется. Новые сырые материалы появятся после
                ответа API, список обновится автоматически.
              </InlineNotice>
            )}
            {collectRss.isError && (
              <InlineNotice tone="error">
                Не удалось собрать RSS:{" "}
                {getRussianErrorMessage(collectRss.error)}
              </InlineNotice>
            )}
            {collectRss.isSuccess && (
              <OperationResult
                title="Сбор RSS завершён"
                summary={`Получено: ${collectRss.data.fetched ?? 0}. Создано новых материалов: ${collectRss.data.created ?? 0}. Пропущено: ${collectRss.data.skipped ?? 0}. Данные источников и сырых материалов обновлены.`}
              />
            )}
            {sources.map((source) => (
              <SourceCard
                key={source.id}
                source={source}
                disabled={collectRss.isPending}
                isCollecting={
                  collectingSourceId === source.id && collectRss.isPending
                }
                onCollect={(item) => {
                  setCollectingSourceId(item.id);
                  showToast({
                    title: "Выполняется…",
                    description: `Собираем RSS из источника «${item.name}».`,
                    kind: "loading",
                  });
                  collectRss.mutate({ source_id: item.id });
                }}
              />
            ))}
