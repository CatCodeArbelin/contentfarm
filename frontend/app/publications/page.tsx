...
function StatusBadge({ status }: { status: ApiStatus }) {
  return (
    <span
      className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusPalette[status] ?? "border-white/15 bg-white/10 text-slate-200"}`}
    >
      {statusText(status)}
    </span>
  );
...
function SelectFilter({
  ...
  return (
    <label className="grid gap-2 text-sm text-slate-300">
      ...
      >
        {options.map((option) => (
          <option key={option || allLabel} value={option}>
            {option || allLabel}
          </option>
        ))}
      </select>
  ...
  return (
    <div className="animate-pulse rounded-3xl border border-white/10 bg-white/[0.06] p-5">
      <div className="flex flex-wrap items-center gap-3">
        <div className="h-7 w-36 rounded-full bg-slate-700/70" />
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {Array.from({ length: 6 }).map((_, index) => (
          <div key={index} className="space-y-2">
            ...
            <div className="h-5 w-full rounded bg-slate-700/70" />
          </div>
        ))}
      </div>
    </div>
...
function ErrorDetails({ error }: { error?: string | null }) {
  ...
  return (
    <div className="rounded-2xl border border-rose-300/25 bg-rose-400/10 p-4 text-rose-100">
      <p className="text-xs font-bold uppercase tracking-[0.2em] text-rose-200/80">
        Ошибка публикации
      </p>
...
function CopyExportPathButton({ exportPath }: { exportPath?: string | null }) {
  ...
  return (
    <button
      ...
      onClick={handleCopy}
      disabled={!canCopy}
      className="rounded-2xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-2 text-sm font-bold text-cyan-100 transition hover:border-cyan-200/50 hover:bg-cyan-300/20 disabled:cursor-not-allowed disabled:border-white/10 disabled:bg-white/[0.04] disabled:text-slate-500"
    >
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="min-w-0 text-sm">
      ...
      <div className="mt-1 break-words font-medium text-slate-100">
        {children}
      </div>
    </div>
  );
    ...
    <article className="rounded-3xl border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-slate-950/30">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-slate-500">
            Вариант #{publication.variant_id}
          </span>
        </div>
        <div className="space-y-2 lg:max-w-sm">
          ...
          >
            {publication.status === "published"
              ? publication.export_path
                ? "Экспорт уже выполнен: путь можно скопировать."
                : publication.message_id
                  ? "Публикация уже отправлена: повторное действие не требуется, message_id сохранён."
                  : "Публикация уже завершена: повторное действие не предлагается основным сценарием."
              : publication.status === "failed"
                ? "Действие недоступно: публикация завершилась ошибкой, подробности указаны в поле error."
                : "Путь экспорта появится после успешного экспорта; сейчас копировать нечего."}
          </InlineNotice>
        </div>
      </div>
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Field label="export_path">
          <code className="rounded-lg bg-slate-950/70 px-2 py-1 text-xs text-cyan-100">
            {valueOrDash(publication.export_path)}
          </code>
        </Field>
        <Field label="message_id">{valueOrDash(publication.message_id)}</Field>
        <Field label="published_at">
          {formatDate(publication.published_at)}
        </Field>
        <Field label="error">
          <ErrorDetails error={publication.error} />
        </Field>
      </div>
    </article>
  );
...
export default function PublicationsPage() {
  ...
  const platforms = [
    "",
    ...Array.from(
      new Set(allPublications.map((item) => item.platform).filter(Boolean)),
    ).sort(),
  ];
    ...
    <main className="min-h-screen bg-slate-950 p-5 text-slate-100">
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="rounded-3xl border border-white/10 bg-white/[0.06] p-6">
          ...
          <h1 className="mt-3 text-3xl font-bold text-white">Публикации</h1>
          <p className="mt-2 text-slate-400">
            Отслеживайте статусы публикаций, экспортированные файлы,
            идентификаторы сообщений и ошибки отправки.
          </p>
        </header>
        ...
        {publicationsQuery.error && (
          <div className="rounded-3xl border border-rose-300/30 bg-rose-400/10 p-5 text-rose-100">
            ...
            {getRussianErrorMessage(publicationsQuery.error)}
          </div>
        )}
        <section className="space-y-3">
          ...
          {!publicationsQuery.isLoading &&
            ...
            publications.length === 0 && (
              <div className="rounded-3xl border border-dashed border-white/15 bg-slate-900/60 p-10 text-center">
                ...
                <p className="mt-2 text-slate-400">
                  После экспорта или отправки материалов записи появятся здесь.
                  Попробуйте изменить фильтры, если ожидаете увидеть
