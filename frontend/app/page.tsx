...
export default function Home() {
  ...
  const trendData = [
    ...
    { label: "Сырьё", value: rawItems.data?.total ?? 0 },
  ...
  return <main className="min-h-screen overflow-hidden bg-slate-950 text-slate-100"><div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(34,211,238,0.22),transparent_28rem),radial-gradient(circle_at_82%_0%,rgba(124,58,237,0.24),transparent_30rem),linear-gradient(180deg,rgba(15,23,42,0),#020617_78%)]" /><div className="relative min-h-screen"><header className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/70 px-5 py-4 backdrop-blur-xl"><div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"><div><p className="text-sm text-cyan-200">Дашборд редакции</p><h1 className="text-2xl font-bold tracking-tight text-white md:text-3xl">Contentfarm: состояние конвейера</h1></div><CommandBar /></div></header><div className="space-y-5 p-5">{isLoading && <LoadingBlock text="Загружаем сводку с сервера…" />}{firstError && <ErrorBlock error={firstError} />}<div className="grid gap-5 md:grid-cols-3">{totals.map((stat) => <GlassCard key={stat.label} className="p-5"><p className="text-3xl font-bold text-white">{stat.value.toLocaleString("ru-RU")}</p><p className="mt-2 text-sm text-slate-400">{stat.label}</p></GlassCard>)}</div><div className="grid gap-5 xl:grid-cols-3"><GlassCard className="p-5 xl:col-span-1"><h2 className="text-xl font-semibold text-white">Общая динамика</h2><p className="mt-1 text-sm text-slate-400">Сравнение объёмов основных сущностей.</p><div className="mt-4 h-52"><ResponsiveContainer width="100%" height="100%"><AreaChart data={trendData}><Area type="monotone" dataKey="value" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.18} /><XAxis dataKey="label" tick={{ fill: "#cbd5e1", fontSize: 11 }} axisLine={false} tickLine={false} /><YAxis tick={{ fill: "#94a3b8", fontSize: 11 }} axisLine={false} tickLine={false} allowDecimals={false} /><Tooltip contentStyle={{ background: "#0f172a", border: "1px solid rgba(255,255,255,.12)", borderRadius: 16 }} /></AreaChart></ResponsiveContainer></div></GlassCard><GlassCard className="p-5"><h2 className="text-xl font-semibold text-white">Варианты по статусам</h2><StatusChart data={statusData(variantsForChart.data?.items ?? [])} /></GlassCard><GlassCard className="p-5"><h2 className="text-xl font-semibold text-white">Публикации по статусам</h2><StatusChart data={statusData(publicationsForChart.data?.items ?? [])} /></GlassCard></div><div className="grid gap-5 xl:grid-cols-[1fr_1fr_22rem]"><LatestList title="Последние варианты" items={latestVariants.data?.items ?? []} getTitle={(item) => item.title || item.lead || `Вариант ${item.id}`} /><LatestList title="Последние публикации" items={latestPublications.data?.items ?? []} getTitle={(item) => item.url || item.export_path || `Публикация ${item.id}`} /><div className="space-y-5"><RecentActions /><GlassCard className="p-5"><h2 className="text-xl font-semibold text-white">Быстрые действия</h2><div className="mt-4 grid gap-3"><a className="rounded-2xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-3 text-sm font-medium text-cyan-100" href="/sources">Добавить источник</a><a className="rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm font-medium text-slate-200" href="/news-events">Открыть инфоповоды</a><a className="rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm font-medium text-slate-200" href="/variants">Открыть модерацию</a><a className="rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm font-medium text-slate-200" href="/publications">Проверить публикации</a><a className="rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm font-medium text-slate-200" href="/settings">Открыть настройки</a></div><p className="mt-4 text-sm leading-6 text-slate-400">Действия ведут в рабочие разделы и не заменяют серверные процессы.</p></GlassCard></div></div></div></div></main>;
}
