...
export type ExportPlatform = "telegram" | "dzen" | "max" | "vc" | "habr" | "dtf" | "pikabu";
...
export type DeduplicateResponse = {
  ...
  items: Array<{ news_event_id: number; source_link_id: number; created: boolean; already_linked: boolean }>;
};
...
export const apiErrorMessages = {
  ...
  network: "Не удалось подключиться к серверу. Проверьте соединение и адрес API.",
...
async function apiRequest<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(backendMessage ? `Ошибка API: ${backendMessage}` : `Ошибка API: сервер вернул статус ${response.status}.`, response.status);
  }
...
export const api = {
  ...
  generateNewsEvent: (newsEventId: number) => apiRequest<GenerateResponse>(`/api/v1/generate/${newsEventId}`, { method: "POST" }),
  getVariants: (params?: ListParams) => apiRequest<PaginatedResponse<Variant>>(`/api/v1/variants${toQueryString(params)}`),
  getVariant: (variantId: number) => apiRequest<Variant>(`/api/v1/variants/${variantId}`),
  approveVariant: ({ variantId, approved_by = "frontend" }: { variantId: number; approved_by?: string }) =>
    apiRequest<Variant>(`/api/v1/variants/${variantId}/approve`, { method: "POST", body: JSON.stringify({ approved_by }) }),
  rejectVariant: ({ variantId, reason, rejected_by = "frontend" }: { variantId: number; reason?: string | null; rejected_by?: string }) =>
    apiRequest<Variant>(`/api/v1/variants/${variantId}/reject`, { method: "POST", body: JSON.stringify({ reason, rejected_by }) }),
  exportVariant: ({ variantId, platform, export_format = "markdown" }: { variantId: number; platform: ExportPlatform; export_format?: ExportFormat }) =>
    apiRequest<Publication>(`/api/v1/publications/${variantId}/export`, { method: "POST", body: JSON.stringify({ platform, format: export_format }) }),
  publishTelegram: (variantId: number) => apiRequest<Publication>(`/api/v1/publications/${variantId}/publish-telegram`, { method: "POST" }),
  getPublications: (params?: ListParams) => apiRequest<PaginatedResponse<Publication>>(`/api/v1/publications${toQueryString(params)}`),
};
...
export const apiKeys = {
  ...
  newsEvents: (params?: NewsEventListParams) => ["news-events", params] as const,
...
type MutationConfig<TData, TVariables> = UseMutationOptions<TData, ApiError, TVariables>;

export function useHealthCheck(options?: QueryConfig<HealthResponse>) {
  return useQuery({ queryKey: apiKeys.health, queryFn: api.healthCheck, ...options });
}

export function useGetSources(params?: ListParams, options?: QueryConfig<PaginatedResponse<Source>>) {
  return useQuery({ queryKey: apiKeys.sources(params), queryFn: () => api.getSources(params), ...options });
}
...
export function useGetRawItems(params?: ListParams, options?: QueryConfig<PaginatedResponse<RawItem>>) {
  return useQuery({ queryKey: apiKeys.rawItems(params), queryFn: () => api.getRawItems(params), ...options });
}

export function useDeduplicate(options?: MutationConfig<DeduplicateResponse, number | undefined>) {
  const queryClient = useQueryClient();
  return useMutation({ ...options, mutationFn: api.deduplicate, onSuccess: (...args) => { void queryClient.invalidateQueries({ queryKey: ["news-events"] }); void queryClient.invalidateQueries({ queryKey: ["raw-items"] }); options?.onSuccess?.(...args); } });
}

export function useGetNewsEvents(params?: NewsEventListParams, options?: QueryConfig<PaginatedResponse<NewsEvent>>) {
  return useQuery({ queryKey: apiKeys.newsEvents(params), queryFn: () => api.getNewsEvents(params), ...options });
}

export function useGenerateNewsEvent(options?: MutationConfig<GenerateResponse, number>) {
  const queryClient = useQueryClient();
  return useMutation({ ...options, mutationFn: api.generateNewsEvent, onSuccess: (...args) => { void queryClient.invalidateQueries({ queryKey: ["variants"] }); options?.onSuccess?.(...args); } });
}

export function useGetVariants(params?: ListParams, options?: QueryConfig<PaginatedResponse<Variant>>) {
  return useQuery({ queryKey: apiKeys.variants(params), queryFn: () => api.getVariants(params), ...options });
}

export function useGetVariant(variantId: number, options?: QueryConfig<Variant>) {
  return useQuery({ queryKey: apiKeys.variant(variantId), queryFn: () => api.getVariant(variantId), enabled: variantId > 0, ...options });
}

export function useApproveVariant(options?: MutationConfig<Variant, { variantId: number; approved_by?: string }>) {
  const queryClient = useQueryClient();
  return useMutation({ ...options, mutationFn: api.approveVariant, onSuccess: (data, ...args) => { void queryClient.invalidateQueries({ queryKey: ["variants"] }); void queryClient.invalidateQueries({ queryKey: apiKeys.variant(data.id) }); options?.onSuccess?.(data, ...args); } });
}

export function useRejectVariant(options?: MutationConfig<Variant, { variantId: number; reason?: string | null; rejected_by?: string }>) {
  const queryClient = useQueryClient();
  return useMutation({ ...options, mutationFn: api.rejectVariant, onSuccess: (data, ...args) => { void queryClient.invalidateQueries({ queryKey: ["variants"] }); void queryClient.invalidateQueries({ queryKey: apiKeys.variant(data.id) }); options?.onSuccess?.(data, ...args); } });
}
...
export function useGetPublications(params?: ListParams, options?: QueryConfig<PaginatedResponse<Publication>>) {
  return useQuery({ queryKey: apiKeys.publications(params), queryFn: () => api.getPublications(params), ...options });
}
